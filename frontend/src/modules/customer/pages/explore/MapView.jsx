...
const MapView = () => {
  ...
  if (!isLoaded) return (
    <div className="min-h-screen flex items-center justify-center bg-[#F8F5FF]">
      <div className="w-8 h-8 border-2 border-[#3D7A4F]/20 border-t-[#3D7A4F] rounded-full animate-spin" />
    </div>
  );

  return (
    <PageTransition>
      <div className="flex flex-col min-h-screen bg-[#F8F5FF] pb-24">
        
        {/* Compact Filters */}
        <div className="sticky top-0 z-[100] bg-[#F8F5FF]/80 backdrop-blur-md px-4 py-3 border-b border-gray-100/50">
        ...
        <div className="px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
             <div className="flex items-center gap-1.5">
               <div className="w-2 h-2 rounded-full bg-[#3D7A4F] shadow-[0_0_6px_#3D7A4F]" />
               <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Active Shops</span>
             </div>
             <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] font-black text-[#3D7A4F] uppercase tracking-tighter">
                ...
                >
                  <div className="text-center p-1 min-w-[140px] font-sans">
                    <div className="relative w-12 h-12 mx-auto mb-2">
                       ...
                       {selected.verified && (
                         <div className="absolute -top-1 -right-1 bg-[#3D7A4F] text-white p-0.5 rounded-full shadow-sm">
                           <CheckCircleRoundedIcon sx={{ fontSize: 10 }} />
                         </div>
                       )}
                    </div>
                    <h4 className="text-[11px] font-black text-gray-900 uppercase tracking-tighter leading-tight">{selected.storeName}</h4>
                    <p className="text-[8px] font-black text-gray-400 uppercase tracking-widest mt-0.5">{selected.category}</p>
                    <div className="flex items-center justify-center gap-1 mt-2 bg-[#3D7A4F]/5 py-1 px-2 rounded-lg">
                       <span className="text-[9px] font-black text-[#3D7A4F]">{offerCounts[selected._id] || 0} ACTIVE OFFERS</span>
                    </div>
                  </div>
                </InfoWindow>
              )}
        ...
        <div className="px-4 mt-6 space-y-4">
          <div className="flex items-center justify-between px-1">
             ...
             {selected && (
               <button onClick={() => setSelected(null)} className="text-[9px] font-black text-[#3D7A4F] uppercase tracking-widest">Reset View</button>
             )}
          </div>
          ...
          <div className="space-y-3">
            {selected ? (
              ...
              <div className="py-12 text-center bg-white rounded-3xl border border-gray-100 shadow-sm">
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">No partners in this area</p>
              </div>
            )}
          </div>
        </div>

...
const FAQItem = ({ question, answer, idx }) => {
  ...
  return (
    <motion.div
      ...
      className={`rounded-2xl border transition-all overflow-hidden ${isOpen ? 'bg-white border-[#3D7A4F]/20 shadow-lg' : 'bg-white/50 border-gray-100 hover:border-gray-200'}`}
    >
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="w-full px-5 py-4 flex items-center justify-between text-left"
      >
        <span className={`text-[13px] font-black tracking-tight ${isOpen ? 'text-[#3D7A4F]' : 'text-gray-800'}`}>{question}</span>
        <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all ${isOpen ? 'bg-[#3D7A4F] text-white rotate-180' : 'bg-gray-100 text-gray-400'}`}>
  ...
  return (
    <div className="min-h-screen bg-[#F8F5FF] pb-24">
      ...
      <div className="sticky top-0 z-50 bg-white/80 backdrop-blur-xl border-b border-gray-100 px-4 py-3 flex items-center gap-3">
        ...
        <h1 className="text-[15px] font-black text-gray-900 uppercase tracking-widest">Support Concierge</h1>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 space-y-10">
        ...
        <div className="bg-gray-900 rounded-[2.5rem] p-6 sm:p-10 relative overflow-hidden shadow-2xl">
           ...
           <div className="relative z-10 flex flex-col sm:flex-row items-center gap-6 lg:gap-10">
              <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="w-16 h-16 bg-white/5 border border-white/10 rounded-2xl flex items-center justify-center flex-shrink-0">
                 <HelpRoundedIcon sx={{ fontSize: 32 }} className="text-[#3D7A4F]" />
              </motion.div>
              <div className="space-y-2 text-center sm:text-left">
                 <motion.h2 initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-xl sm:text-2xl lg:text-3xl font-black text-white tracking-tighter leading-tight">Merchant <span className="text-[#3D7A4F]">Concierge.</span></motion.h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
           {[
              ...
              { label: 'Direct Line', value: '24/7 Priority', icon: CallRoundedIcon, color: 'text-[#3D7A4F]', action: () => window.open('tel:+911800OFFERLY') },
           ].map((c, i) => (
              <motion.button key={i} whileTap={{ scale: 0.98 }} onClick={c.action} className="bg-white p-5 rounded-2xl border border-gray-50 shadow-sm flex items-center gap-4 hover:border-[#3D7A4F]/20 transition-all text-left">
                 <div className={`w-10 h-10 rounded-xl bg-[#F8F5FF] ${c.color} flex items-center justify-center flex-shrink-0`}><c.icon sx={{ fontSize: 20 }} /></div>
                 <div>
                    <p className="text-[12px] font-black text-gray-900 leading-none">{c.label}</p>
        ...
        <div className="space-y-6">
           <div className="flex items-center gap-3 px-1">
              ...
              <h3 className="text-[11px] font-black text-gray-900 uppercase tracking-widest">Help Architecture</h3>
           </div>
           <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
              {[
                 ...
                 <motion.button key={i} whileHover={{ y: -4 }} className="bg-white p-4 rounded-xl border border-gray-50 shadow-sm flex flex-col items-center gap-3 text-center">
                    <div className="w-10 h-10 rounded-lg bg-[#F8F5FF] text-[#3D7A4F] flex items-center justify-center"><cat.icon sx={{ fontSize: 20 }} /></div>
                    <span className="text-[10px] font-black text-gray-600 uppercase tracking-tight">{cat.label}</span>
                 </motion.button>
              ))}
           </div>
        </div>
        ...
        <div className="space-y-6">
           <div className="flex items-center gap-3 px-1">
              <div className="w-1 h-4 bg-[#3D7A4F] rounded-full" />
              <h3 className="text-[11px] font-black text-gray-900 uppercase tracking-widest">Knowledge Base</h3>
           </div>
        ...
        <div className="text-center py-6">
           <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.4em]">Secure Concierge Terminal</p>
        </div>
      </div>
    </div>
  );
